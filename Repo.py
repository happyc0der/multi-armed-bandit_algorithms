...
#: Default value for the constant :math:`C_2`. Should be :math:`>0` and as large as possible, but not too large.
Constant_C2 = 128.0
#: Default value for the constant :math:`C_2`. Should be :math:`>0` and as large as possible, but not too large.
Constant_C2 = 1.0
...
class AdSwitch(BasePolicy):
    ...
    def __init__(self, nbArms,
        ...
        if nbArms > 2:
            ...
            print(
                "FIXME for the generic case of K > 2 arms, use the AdSwitchNew algorithm (paper from 2019)!")
        super(AdSwitch, self).__init__(nbArms, *args, **kwargs)

        # Parameters
        assert horizon is not None, "Error: for a AdSwitch policy, the parameter 'horizon' should be > 0 and not None but was = {}".format(
            horizon)  # DEBUG
        ...
        assert C1 > 0, "Error: for a AdSwitch policy, the parameter 'C1' should be > 0 but was = {}".format(
            C1)  # DEBUG
        self.C1 = C1  #: Parameter :math:`C_1` for the AdSwitch algorithm.

        assert C2 > 0, "Error: for a AdSwitch policy, the parameter 'C2' should be > 0 but was = {}".format(
            C2)  # DEBUG
        ...
        #: Currently exploited arm. It is :math:`\overline{a_k}` in the algorithm.
        ...
        #: Length of the current tests phase, computed as :math:`s_i`, with :func:`compute_di_pi_si`.
        ...
        #: Current best arm, when finishing step 3. Denote :math:`\overline{a_k}` in the algorithm.
        self.current_best_arm = None
        #: Current worst arm, when finishing step 3. Denote :math:`\underline{a_k}` in the algorithm.
        self.current_worst_arm = None
        #: Gap between the current best and worst arms, ie largest gap, when finishing step 3. Denote :math:`\widehat{\Delta_k}` in the algorithm.
        self.current_estimated_gap = None
        #: Memory of the currently used :math:`(d_i, p_i, s_i)`.
    ...
    def find_Ik(self):
        ...
        """
        assert self.current_estimated_gap >= 0, "Error: cannot find Ik if self.current_estimated_gap = {} is not > 0.".format(
            self.current_estimated_gap)  # DEBUG
    ...
    def compute_di_pi_si(self):
        ...
        si_values = self.nbArms * \
            np.ceil((self.C2 * np.log(self.horizon)) / di_values**2)
    ...
    def choice(self):
        ...
        if self.phase == Phase.Estimation:
            ...
            # Round-Robin phase
            self.current_exploration_arm = (
                self.current_exploration_arm + 1) % self.nbArms  # go for next arm
            # Test!
            saw_a_change, sigma = self.statistical_test(
                self.t, self.last_restart_time)
            if saw_a_change:
                mus = [mymean(self.read_range_of_rewards(a, sigma, self.t))
                       for a in range(self.nbArms)]
                self.current_best_arm = np.argmax(mus)
                self.current_worst_arm = np.argmin(mus)
                self.current_estimated_gap = abs(
                    mus[self.current_best_arm] - mus[self.current_worst_arm])
        ...
        # ---------------
            ...
            if self.length_of_current_phase is not None and self.step_of_current_phase < self.length_of_current_phase:
                ...
                self.current_exploitation_arm = (
                    self.current_exploitation_arm + 1) % self.nbArms  # go for next arm
            else:
                if self.current_exploitation_arm is None:
                ...
                if not compute_new_di_pi_si:
                    ...
                    t2 = self.t + 1
                    mus = [np.mean(self.read_range_of_rewards(a, t1, t2))
                           for a in range(self.nbArms)]
                    current_best_mean = np.max(mus)
                    current_worst_mean = np.min(mus)
                    print("Info: the test |mu_a[t1,t2] - mu_b[t1,t2] - Delta| > di/4 is {} for a = best = {}, b = worst = {}, t1 = {} and t2 = {}, Delta = {} and di = {}...".format(
                        abs(current_best_mean - current_worst_mean - self.current_estimated_gap) > di / 4, np.argmax(mus), np.argmin(mus), t1, t2, self.current_estimated_gap, di))  # DEBUG
                    if abs(current_best_mean - current_worst_mean - self.current_estimated_gap) > di / 4:
                        print("Info: the test |mu_a[t1,t2] - mu_b[t1,t2] - Delta| > di/4 was true for a = best = {}, b = worst = {}, t1 = {} and t2 = {}, Delta = {} and di = {}...".format(
                            np.argmax(mus), np.argmin(mus), t1, t2, self.current_estimated_gap, di))  # DEBUG
                ...
                if compute_new_di_pi_si:
                    ...
                    assert 0 <= proba_of_checking < 1, "Error: the sum of pi should be < 1 but it is = {}, impossible to do a Step 5 of Exploitation!".format(
                        proba_of_checking)
            ...
            raise ValueError(
                "Error: AdSwitch should only be in phase Exploration or Checking or Exploitation.")
